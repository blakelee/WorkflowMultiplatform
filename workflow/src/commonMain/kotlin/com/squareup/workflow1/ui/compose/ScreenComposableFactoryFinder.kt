  ...
  public fun <ScreenT : Screen> getComposableFactoryForRendering(
    ...
    return factoryOrNull
      ...
      ?: (rendering as? BodyAndOverlaysScreen<*, *>)?.let {
        ScreenComposableFactory<BodyAndOverlaysScreen<*, *>> { rendering, environment ->
          val innerFactory = rendering.body.toComposableFactory(environment)
          innerFactory.Content(rendering.body, environment)
          // WorkflowRendering(rendering.body, environment)

          rendering.overlays.forEach { overlay: Overlay ->
            (overlay as? FullScreenModal<*>)?.let {
              val factory = ScreenComposableFactory<ComposeScreen> { rendering, environment ->
                rendering.Content(environment)
              }

              factory.Content(overlay.content as ComposeScreen, environment)
            }
          }
        } as ScreenComposableFactory<ScreenT>
      }
